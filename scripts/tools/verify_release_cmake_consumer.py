...
def docker_command(image: str, package_root: Path, source: Path, build: Path, command: list[str]) -> list[str]:
    return [
        "docker", "run", "--rm", "--pull=never", "--network=none",
        "--user", f"{os.getuid()}:{os.getgid()}", "-e", "HOME=/tmp",
        "-v", f"{package_root.resolve()}:/opt/boost-gateway:ro",
        "-v", f"{source.resolve()}:/work/src:ro",
        "-v", f"{build.resolve()}:/work/build",
        image, *command,
    ]
...
def verify_consumer(archive: Path, expected_root: str, image: str, candidate_revision: str) -> dict[str, Any]:
    ...
    if len(image_result) != 2 or image_result[1] != "amd64":
        raise RuntimeError(f"compiler image must be linux/amd64: {' '.join(image_result)}")

    with tempfile.TemporaryDirectory(prefix="boost-gateway-cmake-consumer-") as directory:
        root = Path(directory)
        ...
        checks = [
            ...
            run_check(
                "configure-consumer",
                docker_command(image, package_root, source, build, [
                    "cmake", "-S", "/work/src", "-B", "/work/build", "-G", "Ninja",
                    "-DCMAKE_BUILD_TYPE=Release", "-DCMAKE_PREFIX_PATH=/opt/boost-gateway",
                ]),
            ),
        ]
        if checks[-1]["passed"]:
        ...
        if checks[-1]["passed"]:
            checks.append(run_check(
                "run-consumer",
                docker_command(image, package_root, source, build, ["/work/build/release_sdk_consumer"]),
            ))
    ...
    return {
        ...
        "clean_environment": {
            "image": image, "image_id": image_result[0], "architecture": image_result[1],
            "network": "none", "pull_policy": "never",
...
def main() -> int:
    ...
    try:
        summary = verify_consumer(args.archive.resolve(), args.expected_root, args.image, args.candidate_revision)
    ...
    print(f"release clean CMake consumer: {'PASS' if summary['passed'] else 'FAIL'} "
