...
def validate_h0(checks: list[dict[str, Any]]) -> None:
    ...
    add(checks, f"h0:{workflow}:summary-render", "uses: ./.github/actions/render-validation-summary" in text, "workflow renders GitHub Step Summary through the shared action")
...
def validate_h5(checks: list[dict[str, Any]]) -> None:
    ...
    add(checks, "h5:compatibility-matrix", exists("sdk/docs/compatibility.md") and contains("sdk/docs/compatibility.md", "v3.6.x") and contains("sdk/docs/compatibility.md", "v4.2.1"), "current SDK compatibility matrix exists")
