...
def run_step_with_retries(
    ...
    for index in range(retries + 1):
        ...
        step = run_step(attempt_name, category, command, timeout_seconds)
        ...
        if step.get("status") == "passed":
            ...
            merged["retries_used"] = index
            return merged
        if index < retries:
            time.sleep(retry_delay_seconds)
    failed = dict(attempts[-1])
    failed["name"] = name
    failed["attempts"] = attempts
    failed["retries_used"] = retries
    return failed
...
def main() -> int:
    ...
    steps.append(run_step_with_retries("R1 plain SDK full-flow baseline", "perf_baseline", plain_cmd, args.step_timeout_seconds, retries=3))
    ...
    steps.append(run_step_with_retries("R1 TLS SDK full-flow with server verification", "perf_tls", tls_cmd, args.step_timeout_seconds, retries=3))
    ...
    steps.append(run_step_with_retries("R1 rotated certificate SDK full-flow", "cert_rotation", rotated_cmd, args.step_timeout_seconds, retries=5, retry_delay_seconds=3.0))
