...
def main() -> int:
    ...
    rc = read_text(root, "scripts/verify_release_candidate.py")
    ...
    add(
        ...
        "P3 数据恢复门禁" in checklist and "verify_data_recovery_gate.py" in checklist,
        "release checklist documents the P3 gate",
    )
    add(
        "docs:release-checklist-p4",
        "P4 可观测性/限流门禁" in checklist and "verify_observability_gate.py" in checklist,
        "release checklist documents the P4 gate",
    )
    add(
        "docs:reliability-p3-p4",
        "P3 数据恢复" in reliability and "P4 可观测性/限流" in reliability,
        "reliability matrix documents both P3 and P4",
    )
    add(
        "docs:current-state-p3-p4",
        "P3 数据恢复" in current and "P4 可观测性/限流" in current,
