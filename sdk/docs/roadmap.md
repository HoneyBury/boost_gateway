...
- 后续主要是收口文档口径、固定 runner gRPC workflow 事实和跨语言封装边界，而不是继续补最低限度的 CMake wiring

## 4. 下一步建议

1. 让 SDK 协议层跟 typed `ServiceEnvelope` helper 保持一致
2. 补 SDK 独立文档（README / quickstart / API reference）
3. 在 fixed-runner `BOOST_BUILD_GRPC=ON` workflow 形成真实 summary 前，继续把 generated protobuf/gRPC client 保持为实验接口，而不是升级为默认客户端入口
