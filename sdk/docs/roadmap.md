...
- 实验 `boost_gateway::sdk_grpc` 已进入仓库内 CMake install/export、`find_package()` consumer smoke 和 generated proto header 安装契约
- 文档与示例仍需要继续整理
...
- `sdk/CMakeLists.txt` 已具备安装/导出基础；`boost_gateway::sdk_grpc`、`project_proto`、`gateway.pb.h/gateway.grpc.pb.h` 现也纳入实验安装包
- 后续主要是收口文档口径、维持 fixed-runner gRPC workflow 事实链，以及梳理跨语言封装边界，而不是继续补最低限度的 CMake wiring

## 4. 下一步建议

1. 让 SDK 协议层跟 typed `ServiceEnvelope` helper 保持一致
2. 补 SDK 独立文档（README / quickstart / API reference）
3. fixed-runner `BOOST_BUILD_GRPC=ON` workflow 已有真实 summary（run `29196150703`）；继续把 generated protobuf/gRPC client 保持为实验接口，不因“基础证据已补齐”而直接升级为默认客户端入口
