...
GitHub API 在 2026-07-17 确认 AOI runner 在线，`myserver` 和 `MyDesktop-Win` 离线。
...
后续刷新中，Release run `29564215641`（`18de5ed`）再次通过 clean package/SBOM；增强 R0 run `29564768686`（`d8d8108`）在同一 AOI 上通过 Redis live、runtime HTTP、release baseline、P5/P6 两轮真实 kind 与 N5 SDK，artifact `8400890077` 的顶层 provenance 与 checkout/runner/lockfile 一致。该刷新不改变 `myserver=offline` 的事实，也不构成第二机器证据。
