...
v3.6.2 已完成三平台发布和线上资产复验。本文档保留的大量 pre-freeze run、artifact
和 cache graph 记录用于故障追溯，不表示当前仍有同名发布阻断；当前平台状态以
`docs/platform-production-boundaries.md` 为准，runner 在线状态只以
`docs/runner-inventory.md` 为准。

P2 生产证据 runner 的详细配置、workflow 输入和归档标准见本文档后续章节。

容量、长稳和 release/capacity 必须按 `linux-x64`、`linux-arm64`、`macos-arm64`
分别归档。三平台当前都有原生发布消费事实，但不能继承其它平台的性能阈值、R5 或
ABI/package 结论。历史失败和预冻结记录必须以 artifact 而不是 workflow 名称判断
证据强度。
...
这些是预冻结阶段的历史能力证据。后续 Linux ARM64 / macOS ARM64 已分别完成 Release
`29950040168` / `29950397639`、R0 `29950844165` / `29951222384`、三轮 baseline
`29951531926` / `29952053505`、capacity/R4 `29947599925` / `29948796107` 和原生
2h soak `29952596945` / `29961425142`。Mac soak 包含 Darwin 原生 CPU、内存、
进程树、fd 和线程采样。该批次分属 `46dc79d` 与修复采样器后的 `a01e7bb`，不能
拼接为最终同 SHA 候选。最终 tag 和 published-asset verification 已由 v3.6.2 后续
独立 runs 完成；Mac notarization 仍未声明。
