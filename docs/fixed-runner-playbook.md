...
gRPC workflow 额外提供一次性的 `prepare_local_cache=true` 准入模式：它只从同一 runner 的 legacy Conan home 复制 recipe/source seed，设置不可达代理并以 `--no-remote` 强制用当前 profile 编译全部 native requirements；随后仍必须通过正常的 `--build=never --no-remote` install。该模式不能作为日常构建路径，也不能跨 runner、OS 或架构复制二进制。
