...
```bash
sudo install -d -o "$USER" -g "$(id -gn)" /opt/boost-gateway/conan /opt/boost-gateway/sccache
```

新 runner 只允许在显式预热阶段访问已批准的 Conan remote。每种 runner OS、GCC、架构和 build type 都必须分别预热；成功后，生产证据 workflow 使用 `--no-remote`：

```bash
python3 -m pip install "conan>=2.0,<2.9"
cache_env="$(mktemp)"
...
conan install . --profile:host conan/profiles/linux-gcc-x64 --profile:build conan/profiles/linux-gcc-x64 \
  --lockfile conan/locks/linux-gcc-x64-release-nogrpc-nosqlite.lock \
  -o "&:with_grpc=False" -o "&:with_sqlite=False" --output-folder=build/conan-release-offline --build=missing --no-remote -s build_type=Release
```
...
| 阻断 | 现象 | 解决方式 |
|---|---|---|
| 持久 Conan Home 权限 | runner 用户无法创建 `/opt/boost-gateway`，resolver 失败 | 管理员在每台 runner 执行 `sudo install -d -o "$USER" -g "$(id -gn)" /opt/boost-gateway/conan /opt/boost-gateway/sccache`，再按 lockfile 预热和 `--no-remote` 复验。 |
...
| Docker 磁盘容量 | 本机完成 6 个应用镜像构建后，镜像约 14.5GB、BuildKit cache 约 13.3GB | 导入/构建前保留至少 25GB 可用空间；仅在 bundle 已安全导出后，才可用 `docker builder prune` 回收构建缓存。 |
