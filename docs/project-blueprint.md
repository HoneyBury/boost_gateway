...
| G2 | gRPC/proto 尚未成为默认主链 | `BOOST_BUILD_GRPC=OFF`，gRPC Gateway 已覆盖 Room/Battle/Match/Leaderboard 基础 RPC、SDK-integrated full-flow、Battle 可取消 stream、trusted principal RBAC 与 TLS/mTLS E2E，但仍缺 observability、独立 SDK 安装包契约和 fixed-runner `BOOST_BUILD_GRPC=ON` 证据 | 中期完成 generated proto/gRPC full-flow 的剩余观测/交付/固定 runner 证据，再决定是否进入默认链路 |
...
| `src/v2/service/envelope_adapter.*` | typed envelope 与 legacy raw JSON 兼容层 | 短期保留但冻结 raw JSON；新增服务不得扩展 raw payload | 全部 5 服务域 29 个业务 handler 已统一接入 adapter，且 29 个已具备 schema-backed typed contract（含 login 域 `register_account` / `guest_login` 与 room governance / control-plane 风格消息）；剩余 raw JSON 面已收敛到内部 Raft RPC；兼容窗口可安全冻结 |
