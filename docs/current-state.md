...
P2 的五项下一 minor ADR 已接受，机器可校验状态见 `docs/decisions/v3.6-decision-manifest.json`。P0-P6 的仓库内实现现已完成，逐阶段事实见 `docs/v3.6-implementation-status.md`：Raft 已具备 strict disk/command codec、legacy/protobuf 双读、显式 capability-gated Phase C writer 与 apply fail-closed；身份模块已具备 multi-kid/JWKS snapshot resolver；macOS ARM64、SDK 4.2.0 wheel/NuGet 和 Linux debug-symbol pair 均已有候选构建/验证 workflow。当前本地验证覆盖 macOS native package、安装后 Python 六服务 full-flow，以及 linux/amd64 ELF strip/build-id/debuglink/受控崩溃。exact-SHA Linux/macOS fixed-runner、真实 HTTPS JWKS drill、NuGet clean consumer、不可变发布资产和线上独立复验仍未完成，因此默认生产链路和 manifest 阻断状态不变。
