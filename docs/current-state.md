...
- P2 固定 runner 证据：`.github/workflows/production-evidence.yml` 已支持 JSON runner 输入、preflight summary 归档、R0/P6 证据、Redis/kind 真实依赖、runtime HTTP observability 和 release/capacity baseline；`.github/workflows/long-soak-capacity.yml` 已补 N1 长稳/容量专用入口；新增 `.github/workflows/production-readiness.yml` 通过 production-evidence/long-soak run ID 下载 artifact，在同一 workspace 统一生成 R2/R3，配置说明见 `docs/fixed-runner-playbook.md`。
