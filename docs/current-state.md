...
- 2026-07-10 已再次在 GitHub-hosted `ubuntu-latest` 上通过 `.github/workflows/ci.yml` 的 `workflow_dispatch` 主线验证，确认当前 `develop` 分支在无 self-hosted runner 参与时也能完成 Conan install、Build、CTest、R4 contract、monitoring operability、workflow Python CLI contract gate 和 legacy/helper inventory gate。对应最新治理补充包含：workflow 内 Python 脚本参数漂移静态门禁（`scripts/check_workflow_python_cli_contracts.py`）、`ci.yml` 上的显式 `sccache` 安装/缓存目录预创建，以及针对 hosted warm-cache 失效问题新增的 Conan cache 恢复试点。当前已确认 `.sccache` exact-match restore 正常，但 fresh `CONAN_HOME` 会让 Conan package 前缀从 `.conan2-local/p/b/gtestf45dc96c860e3/p` 漂移到 `.conan2-local/p/b/gtest2e187237cc749/p` 这类新路径，导致跨 run `sccache` 仍然全 miss；下一步需要以 Conan cache 恢复后的 cold/warm 数据作为 hosted baseline 新事实源。
