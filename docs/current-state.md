...
本文档作为当前进度的入口事实源。版本号以 `CMakeLists.txt` 中的 `boost_gateway VERSION 3.5.1` 为准；提交状态以 `git HEAD` 为准。`v3.5.0` 已发布，当前进入 `v3.5.x` 补丁维护线。
...
1. 当前发布主线：`v3.5.0` 已冻结发布；按 `docs/v3.5.x-maintenance-plan.md` 先收束 `v3.5.1` 发布与事实治理，再分别推进真实 Operator kind/发行包和 8h/专项性能证据。`verify_stability_soak.py` 的 long/overnight profile 分别强制不少于 7200/28800 秒。
