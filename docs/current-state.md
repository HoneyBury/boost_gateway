...
- `v3.5.2` SDK/符号分发边界：正式安装面是 C++ CMake package、C ABI shared library 与 Linux x64 tarball/SBOM；Python `pyproject.toml` 和 C# `.csproj` 继续作为轻量源码 wrapper/example，不承诺 wheel/NuGet 仓库分发。当前 Release 只构建 `Release`，没有 `RelWithDebInfo`、符号拆分或符号服务器契约，独立 debug-symbol 包延期到下一次 minor。AOI 的 N5 downstream CMake consumer 已通过 `5/5`，clean Ubuntu runtime consumer 也通过，但 compiler-bearing clean container 的 CMake consumer 尚未形成，不能把两份不同环境证据拼成该结论。
...
1. 当前发布主线：`v3.5.1` 已发布；`v3.5.2` 的 clean package/SBOM 和真实 Operator kind 阶段证据已通过，剩余最终候选 Release/R0、tag attestation 与第二 Linux runner 边界按 `docs/v3.5.x-maintenance-plan.md` 收束。之后再推进 `v3.5.3` 的 8h/专项性能证据；`verify_stability_soak.py` 的 long/overnight profile 分别强制不少于 7200/28800 秒。
