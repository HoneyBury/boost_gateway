...
- 依赖治理 PoC：仓库已新增 `conanfile.py` 与 `BOOST_USE_CONAN_DEPS=ON` 路径，用于 Conan 2 最小 PoC；默认依赖入口仍是 `FetchContent/third_party` fallback。
...
- 仓库已新增独立的 `conan-validate.yml` 手动流水线，用于在不扰动默认 CI 的前提下验证 Conan 依赖链。
