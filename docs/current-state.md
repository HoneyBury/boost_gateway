...
P2 的五项下一 minor ADR 已接受，机器可校验状态见 `docs/decisions/v3.6-decision-manifest.json`，并由 `scripts/check_next_minor_decisions.py` 阻止文档、决策中的默认激活状态和实现顺序漂移。Raft Phase A strict disk codec 与 Phase B 实现候选已落地：默认依赖图独立引入 protobuf runtime，`raft.proto` 覆盖共识和 capability envelope，RPC reader 严格双读 legacy JSON/protobuf v1，peer capability 只接受显式响应且在 peer 回滚/失联时撤销旧缓存，RequestVote/AppendEntries writer 继续固定为 legacy JSON。三节点 protocol-profile E2E、严格 v1-to-v0 工具、内容寻址有界迁移历史和真实 `v3.5.3`/候选三进程十三阶段双周期本地门禁已覆盖逐节点升级/回滚、leader 重选、committed log catch-up、磁盘 schema 轨迹与三节点读回；release workflow 已把混合 binary summary 纳入严格离线 Conan、specialized/data-recovery/package-consumer/SBOM 同 run 聚合门禁。exact SHA 的 Linux fixed-runner 滚动事实、Phase C protobuf writer 和 command codec 仍未完成；JWKS/多 `kid`、正式跨语言 SDK 分发、macOS ARM64 正式平台支持和独立 debug symbols 也尚未完成实现或发布。因此默认生产链路和 manifest 阻断状态不变。
...
6. 下一 minor 实现：Raft Phase A/B、本地完整回归、受控 v1 state downgrade、内容寻址多周期 sidecar 和真实 `v3.5.3`/候选三进程十三阶段门禁已落地；下一步在提交后的 exact SHA 上运行 Linux fixed-runner release gate，再推进 command codec 与 Phase C 显式 protobuf writer。严格按 `docs/decisions/v3.6-decision-manifest.json` 的依赖和退出条件推进；在兼容测试、平台消费验证和发布资产门禁完成前，五项能力继续保持默认阻断。
