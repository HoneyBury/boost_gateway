...
`v3.6.2` annotated tag 固定在 `ac99ae353a2a6e846f934c8d81c78a07f420f683`。tag Release run `30063021104` 的 Linux x64、Linux ARM64、macOS ARM64 package、三 RID NuGet 和 publish 全部成功，GitHub Release 包含 25 个不可变资产：6 个 runtime/symbol archive、3 个 SDK wheel、1 个 NuGet、10 个 SPDX、4 个 SDK provenance metadata 和总 checksum。Linux x64、Linux ARM64、macOS ARM64 线上独立复验 runs `30063950242`、`30063441646`、`30063444082` 均通过 checksum、build-id/UUID、clean consumer、provenance 与 attested SPDX predicate 绑定。PyPI/NuGet.org trusted publishing 和 Apple notarization 仍是独立激活项，不属于本次完成声明。

legacy/helper 迁移边界与 v1 兼容面清单见 `docs/legacy/legacy-helper-inventory.md`。
项目待办以 `docs/todos/tasks.json` 为版本化事实源，`docs/todos/BOARD.md` 提供生成视图；`scripts/manage_todos.py` 负责状态变更与显式 GitHub Issue 同步，CI 会阻断 schema 或看板漂移。
...
- 依赖治理补充：仓库已新增 `scripts/generate_conan_lock.py`、`conan/profiles/linux-gcc-x64`；`release.yml` 与 `long-soak-capacity.yml` 已支持把 Ubuntu fixed-runner 与同一份 `conan_lockfile` 关联使用。当前默认主线 Conan 路径是 `with_grpc=False`、`with_raft_protobuf=True`、`with_sqlite=False`；protobuf runtime 只服务内部 Raft codec，gRPC transport 和 sqlite3 继续保留为实验或可选层。
...
- Conan 主线严格解析 `fmt`、`spdlog`、`nlohmann_json`、`hiredis`、`boost::headers`、`OpenSSL`、Raft 所需 `protobuf` 和测试所需 GTest；`grpc/sqlite3` 仍属实验或可选层。
...
- helper/generated contract 收口补充：全部 5 服务域的 29 个业务 handler 已统一接入 adapter，且 29 个均已具备 `EnvelopeMessageKind` / schema-backed typed request/response 覆盖，包含 login 域补齐后的 `register_account` / `guest_login` 以及 room governance / control-plane 风格消息（`room_list`、`room_detail`、`room_kick`、`room_transfer_owner`、`room_state_push`、`room_battle_finished`）。内部 Raft RPC 已具备严格 legacy JSON/protobuf v1 双读和独立 capability 消息，但 RequestVote/AppendEntries writer 仍固定使用 legacy JSON；配置了非空 `storage_dir` 的 Raft 节点磁盘 state 已使用独立的严格 versioned JSON codec。
...
- 生产认证边界：生产 `auth.mode=external-jwt` 已收口为外部身份提供方的 RS256 JWT 验证方，强制 public key、issuer、audience 与 `exp`，拒绝对称密钥、私钥、本地注册、guest 登录和 refresh token 签发。开发模式仍保留演示身份能力；其进程内存账户状态不再保存任何 credential hash，不能作为生产账户库。v3.6 已提供 static multi-`kid`/JWKS 候选和真实 HTTPS 轮换 workflow；Linux x64 exact-SHA fixed-runner 证据已完成，跨平台冻结候选与正式发布资产完成前仍保持默认阻断。持久化账户体系和可撤销 refresh token 仍属于外部身份提供方集成的后续工作。
...
- 2h/8h soak、capacity 和业务专项已进入固定 runner 阻断证据链；Redis/Raft 恢复、Redis 告警生命周期和 TLS 对照已形成真实专项证据。发布后候选 `37897e8` 已完成真实 lifecycle 饱和曲线、service CPU 与 `io_cores` 两条单变量轴以及 OTel exporter/collector/backend 对账；生产容量上限声明、更完整 Operator rollback/probe E2E、更完整角色化 RBAC、外部 OTel collector 长稳和 generated gRPC transport PoC 仍属于后续专项。默认生产主链仍是 SDK + TCP gateway + BackendEnvelope + 五后端 + Redis。
...
2026-07-22 ARM64 package/security 批次进一步关闭了平台入口缺口。Linux ARM64 JWKS `29926003937` 通过真实 HTTPS drill 10/10 与 contract 6/6；SDK `29926636641` 在 `a355fb7500ad259ae8921db04effbe325483400f` 生成 manylinux aarch64 wheel 与 linux-arm64 NuGet，package 25/25、fresh full-flow 15/15；debug-symbol `29926847088` 在同 SHA 完成 14 ELF、独立验证 116/116 和 crash probe 12/12。macOS ARM64 JWKS `29925779628` 同样通过 10/10 与 6/6；candidate `29927622379` 在 `a355fb7` 完成 strict-offline Conan、全量 CTest、原生 R5、bounded performance/stability、package 与 dSYM 160/160；SDK `29928355843` 完成 osx-arm64 package 23/23、full-flow 15/15。

2026-07-23 ARM 原生生产预冻结批次完成了剩余运行入口。Linux ARM64 与 macOS ARM64 的完整 capacity/R4 分别由 `29947599925` / `29948796107` 通过，Release 由 `29950040168` / `29950397639` 通过，R0 由 `29950844165` / `29951222384` 通过，三轮 performance baseline 由 `29951531926` / `29952053505` 通过。Linux ARM64 2h soak `29952596945` 连续执行 `7200.779s`、完成 2711 轮和 241 个资源样本；修复 Darwin 原生采样后，macOS ARM64 2h soak `29961425142` 在 `a01e7bb3a3d311378eefd16dcbb9e6682debedf9` 连续执行 `7200.010s`、完成 3242 轮和 239 个资源样本，CPU、内存、进程树、fd、线程、覆盖率和连续性检查全部通过。除 Mac soak 外该批次绑定 `46dc79dcea1098d04a5ab07ecedb2c3ee7467cdf`，因此仍是完整的预冻结能力事实，不可组合为最终同 SHA 候选。Mac notarization、冻结 SHA 全量刷新和不可变发布资产复验仍是独立阻断项。

当前默认分支已把 Release、R0、performance、long-soak/capacity、P5/P6、readiness 和 published-asset verification 接入统一三平台原生路由。tag Release 生成三份独立 archive/SBOM，readiness 和发布后复验按平台消费并拒绝跨架构 ELF/OCI/Mach-O。路由与预冻结运行均已验证，但冻结 SHA 和不可变发布资产尚未形成，因此 ARM 平台状态仍保持 production candidate。
...
6. `v3.6.0` 候选冻结：P0-P6 仓库内工作和 ARM 原生预冻结 Release/R0/baseline/capacity/2h soak 已完成。下一步只在本次版本冻结提交上刷新三个平台的 required workflow 与 artifact；随后等待 Mac 签名/notarization 凭据，创建不可变 tag、发布资产并执行独立复验。严格按 `docs/v3.6-implementation-status.md` 的外部证据清单收口；在平台消费、线上资产和回滚门禁完成前，五项能力继续保持默认阻断。
