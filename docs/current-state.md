...
GitHub 仓库当前 runner inventory 的单一事实源见 `docs/runner-inventory.md`；runner 命名、custom labels、Conan/Docker/R5 准入规则见 `docs/runner-gate-standard.md`。截至 2026-07-16 22:00 CST，`aoi-omen-gaming-laptop-16-am0xxx` 在线且空闲，`myserver` 与 `MyDesktop-Win` 离线。AOI 的默认 Release namespace 和实验 gRPC GCC 13 namespace 均已通过严格离线准入；生产证据仍必须以各 summary 和 artifact 为准。
...
- 截至 run `29494894953`，`480d5fd` 已形成 2h 成功事实；最新 R0 是 `4e15342` / `29466984404`，最新完整 R5/R6 是 `18abba2` / `29428322350`，当前可用 R4 成功事实也来自更早提交，均不能跨 SHA 拼接。将本次事实更新提交后，以该文档提交形成的 `480d5fd` 后继提交作为唯一冻结候选，在该完整 SHA 上重新生成 R0、2h、capacity + business-capacity/R4、R5/R6，最后把四类新 run ID 交给 `production-readiness.yml` 生成 R2/R3。
...
1. 当前发布主线：冻结 v3.5.0 候选 SHA，先执行低成本门禁，再在该提交上生成 R0、真实 2h soak/R4、R5/R6，最后由 `production-readiness.yml` 生成 R2/R3。`verify_stability_soak.py` 的 long/overnight profile 分别强制不少于 7200/28800 秒；8h 可在 2h 候选链闭环后作为高风险发布补充，不替代也不阻塞本轮 2h 证据。
