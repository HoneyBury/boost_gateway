...
P2 的五项下一 minor ADR 已接受，机器可校验状态见 `docs/decisions/v3.6-decision-manifest.json`。P0-P6 的仓库内实现现已完成，逐阶段事实见 `docs/v3.6-implementation-status.md`：Raft 已具备 strict disk/command codec、legacy/protobuf 双读、显式 capability-gated Phase C writer 与 apply fail-closed；身份模块已具备 multi-kid/JWKS snapshot resolver；SDK 4.2.0 wheel/NuGet 和 Linux debug-symbol pair 均已有候选构建/验证 workflow。生产平台目标现明确为 `linux-x64`、`linux-arm64`、`macos-arm64` 三条独立证据链，机器契约见 `docs/platform-production-boundaries.json`。Linux ARM64 已有 Conan profile/lockfile 与 `linux/arm64` Docker/R5 参数契约；macOS workflow 已接入 Mach-O 原生 gateway restart R5。本机真实原生 R5 的 19/19 检查通过，重启 RTO 约 0.8 秒，重启前后 SDK full-flow 均成功；这仍是本机能力事实，不替代 exact-SHA GitHub artifact。Linux ARM64 runner/cache/full gates、macOS 平台容量/长稳/发布资产、真实 HTTPS JWKS drill、NuGet clean consumer和线上独立复验仍未完成，因此对应平台保持 production-candidate 或 foundation。
...
6. 下一 minor 实现：P0-P6 仓库内工作已落地；下一步在同一 exact SHA 上依次运行 Linux release/SDK/debug-symbol、macOS ARM64 和真实 JWKS rotation fixed-runner gates，再进入不可变 GitHub Release 资产复验。严格按 `docs/v3.6-implementation-status.md` 的外部证据清单收口；在平台消费、线上资产和回滚门禁完成前，五项能力继续保持默认阻断。
