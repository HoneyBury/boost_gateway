...
- 2026-07-12：fixed-runner 的 8h overnight soak 已完整通过（28815.787s）；此前 long-soak workflow 把 dispatch 的 `false` 误回退为 `true`，额外执行容量步骤。容量与 business-capacity 都稳定复现 `battle-500` P99=750ms（其余连接、失败、吞吐门槛通过）。复核发现采集报告将未设置的 backend pool 误报为 8，而 gateway bridge 的真实默认值是 1，route worker 也仍为默认 4；capacity profile 现显式配置 8 条 backend 连接与 8 个 route worker，P99 门槛仍为 500ms，需在 fixed runner 重跑确认。
- 2026-07-12：性能瓶颈已定位并在本机 Release 定向验证。`InstanceRuntime` 将不同 battle 实例从全局锁拆分为实例级锁；`GatewayServiceBridge` 在连接池未满时真正扩容、满后才轮转复用，避免所有 battle route 串行落在首条连接。本机仅 `battle-500-30s` profile（500 clients、pool=8、route workers=8）得到 P99=100ms、约 6182 msg/s、零拒绝/失败；完整 fixed-runner 三轮容量证据仍需重跑。
...
- 2026-07-11 已在在线 Linux fixed runner 上完成当前 `develop` 的有界主线回归：`release.yml` run `29142235214`、`conan-validate.yml` run `29142663279`、`nightly-stability.yml` run `29142649741`、`ci.yml` run `29143332897`、`perf-regression.yml` run `29143057517` 和 `perf-commit-check.yml` run `29144040776` 均在 `cb1c853` 上成功。专项 E2E run `29145172304` 在 `e8ea86f` 上成功，生产 resilience run `29145497642` 在 `cbed12d` 上成功，生产 evidence run `29146018657` 在 `ecf624c` 上成功。R0 candidate run `29152333112` 在 `8cadbef` 上成功，R0/P5/P6/N5 的归档 summary 均为 `overall_pass=true`，且 Debug candidate 已按 debug baseline 评估。长稳/容量 run `29146495724` 在 `ea05045` 上失败：其 `long` profile 只记录了 13.952 秒的有界执行，不能作为真实 2h soak 声明；capacity 的 battle-500 P99 为 750ms、超过 500ms 门槛；business-capacity 则因 SDK client 子进程非 UTF-8 输出触发 `UnicodeDecodeError`。当前必须先落实真实时长 soak、刷新 capacity/business-capacity 证据，才能形成 R2/R3 完整通过链。
- 2026-07-11 的 long-soak/capacity run `29153158335` 已确认 Conan preflight/build 通过，business-capacity 的 SDK full-flow 三轮通过且不再出现 UTF-8 解码异常；capacity 与 business-capacity 的唯一性能阻断仍是 battle-500 P99=750ms（门槛 500ms）。该 run 的新持续基准在第二轮 gate 失败后 209 秒提前退出，未形成 2h 事实；后续实现已改为持续满目标时长、汇总失败指标，并上传 P5 与 `v2-stability-soak` 子产物。
...
- 2026-07-12 的 long-soak run `29153927305` 已真实持续 7200.241 秒、完成 1603 次架构基准。仅 `multi_battle_tick_100_entities` 有 4 次（0.25%）超过 1000us，最后一次为 1036.3us。持续 soak 现将单样本轻微抖动与持续退化区分：失败率须不超过 1%，且最差偏离须不超过 20%；超出任一限制才阻断。此统计规则只适用于重复微基准，capacity/business-capacity 的 battle-500 P99=750ms 仍是硬性阻断。
...
- P2 固定 runner 证据：`.github/workflows/production-evidence.yml` 已支持 JSON runner 输入、preflight summary 归档、P6 证据、Redis/kind 真实依赖、runtime HTTP observability 和 release/capacity baseline；新增独立 `.github/workflows/production-candidate-evidence.yml` 生成 R0，避免与 P6 在同一 workspace 重复门禁；`.github/workflows/production-readiness.yml` 通过 production-candidate/long-soak run ID 下载 artifact，在同一 workspace 统一生成 R2/R3，配置说明见 `docs/fixed-runner-playbook.md`。
...
- R2 生产候选证据 Manifest：`docs/production/production-candidate-evidence-manifest.json` 与 `scripts/check_production_evidence_manifest.py` 已将 R0/R1 本机有界证据、固定 runner long-soak/capacity、release/capacity、预发恢复演练和 TLS 预发多轮证据统一成可校验 manifest。R0 fixed-runner aggregate 已由 run `29152333112` 成功补齐；R2/R3 仍被真实时长 long-soak、capacity/business-capacity 及其 R4 summary 阻断，`--require-fixed-runner` 继续作为投产前阻断项。
