...
本文档作为当前进度的入口事实源。版本号以 `CMakeLists.txt` 中的 `boost_gateway VERSION 3.5.2` 为准；提交状态以 `git HEAD` 为准。`v3.5.2` 已发布，后续维护范围按 `docs/v3.5.x-maintenance-plan.md` 执行。
...
1. 当前发布主线：`v3.5.2` 已完成同 SHA Release/R0、tag attestations、第二 Linux runner 与双 runner 线上资产验收并正式发布。之后推进 `v3.5.3` 的 8h/专项性能证据；`verify_stability_soak.py` 的 long/overnight profile 分别强制不少于 7200/28800 秒。
