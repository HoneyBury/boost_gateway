...
- 生产认证边界：生产 `auth.mode=external-jwt` 已收口为外部身份提供方的 RS256 JWT 验证方，强制 public key、issuer、audience 与 `exp`，拒绝对称密钥、私钥、本地注册、guest 登录和 refresh token 签发。开发模式仍保留演示身份能力；其进程内存账户状态不再保存任何 credential hash，不能作为生产账户库。v3.6 已提供 static multi-`kid`/JWKS 候选和真实 HTTPS 轮换 workflow，但在 exact-SHA fixed-runner 证据完成前保持默认阻断；持久化账户体系和可撤销 refresh token 仍属于外部身份提供方集成的后续工作。
...
P2 的五项下一 minor ADR 已接受，机器可校验状态见 `docs/decisions/v3.6-decision-manifest.json`。P0-P6 的仓库内实现现已完成，逐阶段事实见 `docs/v3.6-implementation-status.md`：Raft 已具备 strict disk/command codec、legacy/protobuf 双读、显式 capability-gated Phase C writer 与 apply fail-closed；身份模块已具备 multi-kid/JWKS snapshot resolver；SDK 4.2.0 wheel/NuGet 和 Linux debug-symbol pair 均已有候选构建/验证 workflow。生产平台目标现明确为 `linux-x64`、`linux-arm64`、`macos-arm64` 三条独立证据链，机器契约见 `docs/platform-production-boundaries.json`。Linux ARM64 已有 Conan profile/lockfile 与 `linux/arm64` Docker/R5 参数契约。macOS run `29898739439` 已在 exact SHA `c5dccea425376816196c6ea07f26e4bb3787b813` 完成 strict-offline Conan、Release build/CTest、Mach-O 原生 gateway restart R5、install/package、SPDX 和 checksum；R5 summary 为 `overall_pass=true`，RTO `0.808s`，provenance 正确绑定 macOS ARM64 lockfile。该事实关闭 macOS build/package/native-R5 缺口，但平台容量/长稳、正式 release asset 和线上独立复验仍未完成。Linux ARM64 runner/cache/full gates、Linux fixed-runner JWKS artifact 和 NuGet clean consumer 也仍待闭环，因此对应平台继续保持 production-candidate 或 foundation。
...
6. 下一 minor 实现：P0-P6 仓库内工作已落地，macOS ARM64 build/package/native-R5 exact-SHA artifact 已通过。下一步先让新增 `jwks-rotation.yml` 进入默认分支并在 Linux fixed runner 生成同 SHA artifact，再依次运行 Linux release/SDK/debug-symbol 和 Linux ARM64 runner/cache/R5，最后进入分平台容量基线及不可变 GitHub Release 资产复验。严格按 `docs/v3.6-implementation-status.md` 的外部证据清单收口；在平台消费、线上资产和回滚门禁完成前，五项能力继续保持默认阻断。
