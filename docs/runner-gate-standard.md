...
| Linux ARM64 production R5 | ARM64 ELF、`linux/arm64` Compose images、相同语义的 restart/full-flow/snapshot | runner online，Release G2 已通过；等待 Debug/gRPC cache、image preflight 与 R5 artifact |
| macOS ARM64 production R5 | Apple Clang Mach-O server 原生编排、gateway restart 前后 native SDK full-flow | run `29902706777` 已形成预冻结候选 artifact；完整 baseline/soak/capacity 仍待完成 |
