...
| Linux ARM64 production R5 | ARM64 ELF、`linux/arm64` Compose images、相同语义的 restart/full-flow/snapshot | profile、lockfile、bundle/preflight 参数已实现；等待 runner/cache/run |
| macOS ARM64 production R5 | Apple Clang Mach-O server 原生编排、gateway restart 前后 native SDK full-flow | 已接入 `macos-arm64.yml`；等待线上 artifact 验收 |
