...
本文件对应维护路线 **`v1.1.5`**（参见 `development-optimization.md`「第一步：先统一业务事实源与模块定位」），在 **`../history-v1/v1-maturity-matrix.md`** 的逐项表格之上，用文字说明 **login / room / battle** 三条业务线在 `v1.x` 中的**边界、事实源与派生视图**，避免「各 service 都能改状态、文档各写各的」继续扩散。
...
- **`v2.0.0`** 的 Actor/ECS/集群等**不在** `v1.1.5` 讨论范围；请参阅 `../history-v2/v2-roadmap.md` 且仅在 **`v1.2.0` 决策点**之后启动相关开发。
