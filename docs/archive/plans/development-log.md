...
将仓库从"v1.0.0 发布版 + develop 持续叠加能力"的混合状态，校准为一份"单一事实源、成熟度表述准确"的维护期基线。**对应 `../plans/development-optimization.md` §11 任务表的 T01 / T02 / T10 / T12 / T14**。本阶段为**纯文档基线校准**，不修改任何主链协议 / 业务 / 运行时行为。

### 完成内容

- 新增 `../history-v1/v1-maturity-matrix.md` — `v1.x` 维护期能力成熟度**单一事实源**：
...
- `../plans/development-priority.md`：从"扩展/优化/测试方向"散列优先级，重写为按 `development-optimization.md` §11 任务表 + 版本批次（A 基线 / B 主链 / C 测试 / D 升级决策）组织
...
- `README.md`
- `CHANGELOG.md`
- `../history-v1/v1-maturity-matrix.md`（新）
- `../runbooks/runtime-playbook.md`
- `../plans/development-priority.md`
...
收敛主链生命周期与协议增强路径，让 `Session` 的关闭路径只有一条收口，让协议增强标志位的语义跨 build 自洽。**对应 `../plans/development-optimization.md` §11 任务表的 T03 / T04**。本阶段允许动主链代码，但不引入新功能，不修改业务协议，不动配置/治理结构。
...
- T04：固定协议增强顺序 + 修正压缩标志位语义
  ...
  - `src/net/session.cpp`：
    ...
    - 分片（`packet::flags::kFragment*`）在 v1.x 维护期内仍为 `reserved`：`Session` 主链既不分片也不组帧，相关注释指向 `../history-v1/v1-maturity-matrix.md` §2.3 与 `development-optimization.md` §6.A / §8.3。
...
- 没有压缩后端的 build 上，`kCompressed` 现在是“严格非法”信号；任何在 `v1.0.0` 主链外私接的旧客户端如果在没有 zlib 的 build 上误设了 `kCompressed`，将会被立即断开。这与 `../history-v1/v1-maturity-matrix.md` 的现行约束一致——v1.x 主链就不依赖压缩。
...
**对应 `../plans/development-optimization.md` §11 任务表的 T05 / §8.4**。本阶段不改变业务协议与白名单所列消息号集合，只改变执行位置与资源占用特征。
...
- `../runbooks/runtime-playbook.md` §3、`../history-v1/v1-maturity-matrix.md` §2.4、`../plans/development-priority.md`、`CHANGELOG.md`
...
- 新增 **`../history-v1/v1-business-fact-source.md`**。
- **`docs/README.md`**、`../history-v1/v1-maturity-matrix.md` §3、`development-priority.md`、`runtime-playbook.md`、`CHANGELOG.md` 同步指针。
...
> **强约束**：未进入 v2；未改 `../history-v2/v2-roadmap.md`。
...
- **`../history-v1/v1-string-protocol.md`**（冻结表 + `net::msg` 分叉）。
...
- `../runbooks/runtime-playbook.md`、`v1-maturity-matrix.md`、`development-priority.md`、`docs/README.md`、`CHANGELOG.md` 同步。
...
落实 **`development-optimization.md` §11** 的 **T07 / T08** 与§「第三步」叙述：顶号恢复链、空房 battle 清理链有**统一策略函数**与 **`../history-v1/v1-cross-domain-flows.md`**，避免 `GatewayServer` 与 `RoomService` 各维护一份「空房删战斗」逻辑。
...
- **`../history-v1/v1-cross-domain-flows.md`**；`docs/README.md`、`v1-string-protocol.md` 尾部、`development-priority.md`、`v1-maturity-matrix.md`、`runtime-playbook.md`、`CHANGELOG.md`。
...
- **`../history-v1/v1-room-battle-boundary.md`**；`v1-string-protocol.md`、`v1-cross-domain-flows.md`、`README.md`、矩阵 §3.2、`development-priority.md`、`runtime-playbook.md`、`CHANGELOG.md`。
...
- **`../history-v1/v1-governance-layers.md`**（L0–L3、路径归类、Admin demo-only 定位、版本边界）。
...
落实 **`development-optimization.md`** 路线图**第二步**：停止「协议号已存在、example 已演示、就被默认为正式能力」的扩散；与 **`v1-maturity-matrix.md` §4** 一致的表述写进 **`../history-v1/v1-governance-layers.md` §6**，并校准 **README / playbook / showcase 示例**。

### 完成内容

- **`../history-v1/v1-governance-layers.md` §6**；矩阵 §4 引言、§10 版本表 **`v1.1.10` 当前**；`development-priority.md`（新增维护行 + 节奏）；`docs/README.md`；`v1-string-protocol.md` / `v1-cross-domain-flows.md`；`runtime-playbook.md`；根 `README.md`（含 ctest **66** 与 HTTP 用词）；`examples/admin_demo`、`examples/login_demo`；`admin_service.h` 注释；`CHANGELOG.md`。
...
- **`../history-v1/v1-admin-audit-rules.md`**。
...
落实 **T12**：把 **`GatewayAppConfig`** 的启动/热更新/预留口径从矩阵 **抽成可读运维文档**，矩阵 §5.1 保留锚点并指向 **`../history-v1/v1-config-maturity.md`**；明确 **`ConfigWatcher`** 当前**不是**完整热更新框架。

### 完成内容

- **`../history-v1/v1-config-maturity.md`**；`../history-v1/v1-maturity-matrix.md` §5.1 / §10；`docs/README.md`、`development-priority.md`、`runtime-playbook.md`、`v1-string-protocol.md`、`v1-cross-domain-flows.md`、`CHANGELOG.md`。
- **`../history-v2/v2-design.md`**：**入库**（v2 草案，决策点前不实施）。
...
- **`../history-v1/v1-runtime-lifecycle.md`**；矩阵 §5 引言、§5.3、§10；`docs/README.md`、`../history-v1/v1-config-maturity.md` §6、`development-priority.md`、`runtime-playbook.md`、`v1-string-protocol.md`、`v1-cross-domain-flows.md`、`CHANGELOG.md`。
...
- **文档**：**`../history-v1/v1-runtime-lifecycle.md`**（§5–§8 顺序；**§6** reload 表；**§7** shutdown 表）；矩阵 §5.2 / §5.3 / §10；`development-priority.md`、`runtime-playbook.md`、`v1-string-protocol.md`、`v1-cross-domain-flows.md`、`v1-config-maturity.md` §6、`docs/README.md`、`CHANGELOG.md`。
...
- **新增** **`../history-v1/v1-cross-cutting-capabilities.md`**；矩阵 §6 引言、§4.4 指针、§10；`docs/README.md`、`development-priority.md`、`runtime-playbook.md`、`v1-string-protocol.md`、`v1-cross-domain-flows.md`、`v1-config-maturity.md` §5、`CHANGELOG.md`。
...
- **新增** **`../history-v1/v1-cross-cutting-lifecycle-binding.md`**；更新 **`../history-v1/v1-cross-cutting-capabilities.md`** 与 roadmap 指针；**`../history-v1/v1-runtime-lifecycle.md`** §1；矩阵 §6 引言、§10；`docs/README.md`、`development-priority.md`、`runtime-playbook.md`、`v1-string-protocol.md`、`v1-cross-domain-flows.md`、`v1-config-maturity.md` §5、`CHANGELOG.md`。
...
- **新增** **`../history-v1/v1-cross-cutting-data-formats.md`**；矩阵 §6 引言、§4.4、§10；**`../history-v1/v1-runtime-lifecycle.md`** §1；`docs/README.md`、`development-priority.md`、`runtime-playbook.md`、`v1-string-protocol.md`、`v1-cross-domain-flows.md`、`v1-config-maturity.md` §5、横切 **§5 / §6** 指针、`CHANGELOG.md`。
...
- **T21 / 决策记录**
  - **新增** `../history-v1/v1-structure-upgrade-decision.md`：结论是**当前维护分支不推进** typed protocol / internal bus / battle replay 转正，继续维持 `v1.x` 维护收束边界。
- 同步 `docs/README.md`、`../plans/development-priority.md`、`CHANGELOG.md`。
...
- **E6 — 客户端协议规范**：输出 `../history-v2/v2-protocol-spec.md`
...
- 文档整合：`../history-v2/v2-design.md`、`docs/v3-environment-roadmap.md`、`../history-v2/v2-enterprise-roadmap.md` 等
...
- 验证入口：
  ...
  - `docs/p4-validation-checklist.md`、`../plans/k8s-operator-implementation.md` 同步更新
...
- 文档：
  - 顶层 `README.md`
  - `docs/README.md`
  - `../history-v2/v2-enterprise-roadmap.md`
  - `docs/p4-validation-checklist.md`
  - `../plans/k8s-operator-implementation.md`
  - `proto/README.md`
...
### R1 Windows 后端 accept/read timeout 收口（2026-05-17）

- `BackendServer` 从单一 `session_socket_` 改为多会话跟踪：
  - `session_sockets_` 记录活动 socket，`stop()` 会关闭所有活动连接。
  - `session_threads_` 记录每个会话处理线程，`stop()` 会统一 join，避免测试退出时留下阻塞线程。
- plain TCP `read_frame()` 改为基于 Boost.Asio non-blocking `read_some()` 的 bounded read：
  - header 和 payload 都按同一个 timeout 截止。
  - 不再依赖平台 `select()` 路径，降低 Windows/MSVC 下后端 timeout/recovery smoke 悬挂风险。
- SSL overload 仍使用 blocking read，本次仅修正文档注释，避免把 timeout 行为误写为已实现。
- `docs/current-state.md` 和 `docs/reliability-matrix.md` 已同步新增对应稳定性事实与证据入口。

---

## 2026-05-23 阶段 v3.4.0：实时实例框架 + ECS 管线增强 + 坦克大战 Demo + 业务插件 SPI

### 目标

完成实时实例框架（P3）的完整实现与 SPI 定义，补齐通用身份/房间服务（P1/P2），实现坦克大战 Demo 服务端仿真（P4），并对接结算/排行榜（P5）、断线重连（P6）、性能回归（P7）。在框架层面新增弹道系统（ProjectileSystem）、ECS 生命周期管理（BattleLifecycleSystem）、回放录制（BattleReplaySystem）、AOI 兴趣管理（AoiSystem），以及数据持久化缓存层（CachedBattleDataStore）。

### 完成内容

#### 阶段 A：Realtime Instance Runtime 框架（P3）

- **`include/v2/realtime/types.h`**：定义 `InstanceState`（6 状态）、`FinishReason`（7 种）、`InputEnvelope`、`InputResult`、`PlayerContext`、`InstanceContext`、`FrameContext`、`Snapshot`、`SettlementContext`、`TickStats`
- **`include/v2/realtime/instance_plugin.h`**：`InstancePlugin` SPI 定义 8 个虚方法（`on_instance_created`、`on_player_join`、`on_player_leave`、`on_input`、`on_tick`、`build_snapshot`、`build_settlement`、`build_resume_snapshot`），hot-path 方法标记 noexcept，框架通过 try-catch 包裹实现错误隔离
- **`include/v2/realtime/instance_runtime.h` / `src/v2/realtime/instance_runtime.cpp`**（+429 行）：`InstanceRuntime` 管理实例生命周期（create/destroy/tick）、输入队列、snapshot push、resume snapshot；`register_plugin()` / `create_instance()` / `tick_instance()` / `tick_all()` / `get_resume_snapshot()` / `find_instance()`
- **SPI 合规测试**：`tests/v2/unit/realtime/spi_compliance_test.cpp`（446 行）覆盖异常安全、错误隔离、边界条件

#### 阶段 B：通用身份与房间服务（P1/P2）

- **Identity 注册**：`src/v2/login/login_backend_service.cpp` 增加通用 `register_account` handler，定义 error codes；`tests/v2/unit/identity_register_test.cpp`（112 行）
- **Lobby/Room 补齐**：`src/v2/room/room_backend_service.cpp` 增加 `list_rooms`（分页）、`room_detail`、`kick_member`、`transfer_owner`；room metadata 支持；`tests/v2/unit/room_lobby_test.cpp`（169 行）

#### 阶段 C：坦克大战 Demo 服务端（P4）

- **`demo/games/tank_battle/`**：完整 demo 目录结构（README、rules、protocol、acceptance 文档），CMake `BOOST_BUILD_TANK_DEMO=OFF` 保护
- **TankWorld**：`demo/games/tank_battle/server/tank_simulation/tank_world.h/.cpp`（446 行）确定性 20×15 网格，含坦克移动、碰撞检测、子弹系统、HP/得分
- **TankPlugin**：`demo/games/tank_battle/server/tank_plugin/tank_plugin.h/.cpp` 适配 TankWorld 到 InstancePlugin SPI
- **tank_battle_main**：`demo/games/tank_battle/server/tank_battle_main.cpp`（156 行）独立服务端入口
- **测试**：`simulation_test.cpp`（268 行）、`e2e_test.cpp`（165 行）、`perf_test.cpp`（220 行，500 实例 × 50 ticks）
- **验证脚本**：`demo/games/tank_battle/scripts/verify_tank_battle_demo.py`，覆盖全生命周期

#### 阶段 D：生产硬化与 SDK 全流

- **H0-H1**：ConfigWatcher 热加载、CircuitBreaker 完善、优雅关闭、SessionManager 隔离
- **SDK 全流**：C++ SDK heartbeat/disconnect callback、Python/C# wrapper、`demo_full_flow_client.py`（579 行）覆盖 login→room→battle→move→finish→leaderboard 全生命周期
- **Anti-cheat 增强**：`include/v2/security/anti_cheat.h` / `src/v2/security/anti_cheat.cpp`（106 行）统计异常检测、伤害验证；接入 MovementSystem 和 CombatSystem
- **Raft 生产**：leader 选举日志复制、3 节点集群验证（`raft_e2e_test.cpp` 220 行）、follower 滞后追平
- **CI 工作流**：`perf-regression.yml`、`perf-commit-check.yml`、`release-baseline.yml`
- **多进程集成**：`process_supervisor_test.cpp`（230 行）、`windows_platform_test.cpp`（437 行）、`matchmaking_e2e_test.cpp`（376 行）、`cluster_router_e2e_test.cpp`（356 行）

#### 阶段 E（Round 4）：Remote Actor + TankBattlePlugin + 弹道系统

- **`src/v3/cluster/remote_actor.cpp`**（43 行）：`RemoteActorRef::tell()` 实现跨节点消息投递，`ActorLocationRegistry` 本地查找 + `RemoteActorTransport` TCP 转发
- **`include/v2/battle/tank_battle_plugin.h` / `src/v2/battle/tank_battle_plugin.cpp`**（462 行）：完整 InstancePlugin 实现，注册 4 ECS 系统（MovementSystem、CombatSystem、ProjectileSystem、BattleClockSystem），处理 move/attack/shoot/finish 输入，JSON 快照/结算/恢复
- **`ProjectileSystem`**（`include/v2/battle/game_systems.h` / `src/v2/battle/game_systems.cpp` 166 行）：弹道飞行（速度插值）、单目标伤害、AoE 范围伤害（aoe_radius）、Damage-over-Time（duration_frames）
- **`ProjectileComponent` / `DamageOverlayComponent`**：ECS 组件定义，支持弹道生命周期与 DoT 效果
- **测试**：`tank_battle_plugin_test.cpp`（461 行、19 测试）、`projectile_system_test.cpp`（759 行）、`remote_actor_test.cpp`（106 行）、`redis_persistence_test.cpp`（1106 行）、`gateway_grpc_test.cpp`（270 行）

#### 阶段 F（Round 5）：ECS 生命周期、回放、AOI、文件存储

- **`BattleLifecycleSystem`**（`src/v2/battle/runtime_world.cpp`）：自动状态机 kCreated→kRunning→kFinished，空闲超时（300 帧），全员离线超时（60 帧），kFinished 后清理弹道/DoT 组件
- **`BattleReplaySystem`**（`src/v2/battle/runtime_world.cpp`）：逐帧捕获 frame_number、lifecycle、全部参与者位置/HP/分数/在线状态到 `frame_snapshots` 向量
- **`AoiSystem`**（`src/v2/aoi/aoi_system.cpp` / `include/v2/aoi/aoi_system.h`）：ECS 集成 AOI，`AoiViewComponent` 管理可见实体列表，`SpatialGrid` 空间查询
- **`JsonFileBattleDataStore`**（`include/v2/gateway/battle_data_store.h`）：文件落地 JSON 存储（replays/results/snapshots 子目录）
- **`CachedBattleDataStore`**（`include/v2/data/cached_data_store.h` / `src/v2/data/cached_data_store.cpp`）：LRU 读缓存（1000 条目）+ WriteBehind 异步写，接入 demo_server 归档链路
- **TankBattlePlugin shoot 动作**：解析 target_user_id、aoe_radius、duration_frames、speed，计算弹道，调用 `ProjectileSystem::spawn_projectile()`
- **测试**：`battle_lifecycle_test.cpp`（194 行 8 测试）、`battle_replay_test.cpp`（142 行 11 测试）、`aoi_system_test.cpp`（320 行 10 测试）、`aoi_spatial_grid_test.cpp`（12 测试）、`json_file_store_test.cpp`（379 行 18 测试）

### 影响范围

- **新增文件**：~50 个新源文件（include/v2/realtime/*、include/v2/battle/tank_battle_plugin.*、demo/games/tank_battle/*、include/v2/data/cached_data_store.*、include/v2/aoi/*、include/v2/persistence/*、include/v2/perf/*、tests/v2/unit/realtime/*、tests/v2/unit/tank_battle_plugin_test.cpp 等）
- **修改文件**：CMakeLists.txt（版本 3.3.2→3.4.0）、runtime_components.h、runtime_world.h、runtime_world.cpp、game_systems.h、game_systems.cpp、gateway_service_bridge.cpp、demo_server.h/cpp、runtime.h/cpp
- **文档**：current-state.md、architecture-overview.md、development-log.md、v2-roadmap.md 同步更新

### 测试结果

- 累计 842+ tests 通过（较 v3.3.2 基线新增 62+ 单元测试）
- 3 个预存挂起问题（InvalidJsonPayloadIsHandled、BattleActorRestoreReturnsFalseWithMalformedData、WorldSnapshotRestoreInvalidJsonFails）为非本次引入的 JSON 解析路径死循环，需独立修复

### 风险与问题

- TankBattlePlugin 位于 `src/v2/battle/`（框架目录），需确保后续不引入 `Tank`/`Bullet` 类名污染框架公共 API
- `demo/games/tank_battle/` 的 TankWorld 实现未接入 InstancePlugin SPI，为独立仿真路径
- 弹道系统在 plugin 测试中使用 `static std::atomic<std::uint64_t>` 生成 ID，非生产级 UUID
- ParallelSystemExecutor 已实现但未接入默认 `create_battle_world()` 系统注册流程

### 下一步

- 框架/业务边界常态化：将 TankBattlePlugin 移至 `demo/games/` 或明确标记为 SPI 参考实现
- ParallelSystemExecutor 接入 battle_world 系统注册
- 弹道 ID 生成改为生产级 UUID
- 修复 3 个预存 JSON 解析挂起问题
- 排查 battle-runtime 集成长稳（500 实例 × 500 ticks）
- 将 typed envelope helper 推进到 generated protobuf/gRPC stub
