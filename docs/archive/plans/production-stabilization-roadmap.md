...
- [x] 已固化固定 runner 长稳 / 容量聚合入口：`scripts/run_long_soak_capacity.py` 统一输出 `summary_version=2`、`overall_pass`、`environment`、`artifacts`，并归档 2h/8h soak 与 capacity summary。
- [ ] 在固定机器上执行真实 wall-clock 2h / 8h soak，记录 RSS、fd、CPU、错误率、连接数、P50/P90/P99 和业务成功率；当前 OrbStack / Docker 环境可先持续刷新 2h soak 和 capacity 证据，8h 继续作为后续专项。
...
- [x] 已固化云端/固定 runner 聚合入口：`scripts/run_cloud_production_closure.py` 统一归档 preflight、deploy operability、docker snapshot、kind gate 和 production evidence summary。
- [x] 当前 macOS + OrbStack 环境已通过 `preprod-recovery-drill-summary.json` 完成真实 Docker gateway restart 演练收束；K8s / 更多故障场景继续按固定 runner 周期性补充。
...
由服务端部署治理，对客户端保持透明。2026-05-24 最后一轮已完成真实收口：
package consumer、business-flow、plain real gateway full-flow 与 backend TLS real gateway
full-flow 全部通过，汇总证据为
...
- [ ] 正式 wheel/NuGet 签名与多平台包仓库发布仍为后续客户端发布专项，不作为当前 N5 有界收口阻断项。
...
- [x] 固定 runner 入口、runner JSON 输入、preflight summary、Step Summary 渲染和 artifact 归档已接入 release-baseline / specialized-e2e / production-resilience / production-evidence workflows。
- [ ] 固定 runner / 预发环境继续追加 Redis live、Operator kind、runtime HTTP、release/capacity baseline 和更长 soak 的周期性刷新。
...
当前收束状态：已新增 `scripts/verify_preprod_recovery_drill.py`，在 Docker 可用且镜像已存在时执行真实 Docker Compose gateway restart 演练：启动生产栈、等待 gateway ready、重启 gateway、再次等待 ready、运行 SDK full-flow、采集 Docker production snapshot，并生成/校验 recovery drill record。2026-05-20 已在当前 macOS + OrbStack 环境完成真实复测并通过，`runtime/validation/preprod-recovery-drill-summary.json` 已转绿。
...
- [ ] 在云端预发或固定 Docker/K8s 环境补 gateway/backend/Redis/rollback 多场景记录，继续把本机通过结果扩展为更接近正式生产的预演证据。
...
当前收束状态：已新增 `scripts/verify_tls_preprod_multi_run.py`，多次调用 R1 TLS readiness，聚合 TLS full-flow、证书轮换、CA mismatch expected failure 和 plain-vs-TLS overhead ratio，输出 `runtime/validation/tls-preprod-multi-run-summary.json`。2026-05-20 已在当前授权环境完成 2 轮多轮验证并通过。

本机验证注意：R6 仍需要本机端口绑定权限；在普通沙箱环境下会在 `reserve_free_port()` 阶段触发 `Operation not permitted`，因此后续继续刷新 R6 evidence 时应在已授权的本机或固定 runner 上执行。
