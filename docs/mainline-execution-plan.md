...
8. Linux x64 已在 `d687b9e` 完成 Release/R0/R5-R6，并在 `00ce82e0` 完成专用 JWKS、SDK wheel/NuGet 和 debug-symbol exact-SHA artifact；Linux ARM64 与 macOS ARM64 也已在 `19b1a67` / `a355fb7` 完成各自 JWKS、SDK、符号证据和 Mac 原生 candidate。剩余生产 workflow 已在 ARM 分支接入统一三平台原生路由；下一步合入默认分支，补齐 ARM64 原生 v3.5.3 legacy binaries、Release/R0、完整 baseline/soak/capacity 和 Mac notarization，再冻结一个 SHA 刷新三平台证据并独立复验 GitHub Release 资产。Raft protobuf writer 只允许显式配置且全 peer capability 成立时启用；任何能力撤销都必须回落 legacy writer。
