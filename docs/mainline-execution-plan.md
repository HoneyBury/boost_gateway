...
3. 生产认证边界已经收口：生产 login backend 只验证外部 RS256 JWT，并拒绝本地身份操作。当前代码侧 gRPC observability、安装包契约和 fixed-runner 入口已经补齐；下一优先级转为回填真实 fixed-runner `BOOST_BUILD_GRPC=ON` run 结果，并在结果稳定前继续保持 `defer_default_transport`。
