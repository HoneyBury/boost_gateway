...
| `next_minor_decision_gate` | bounded | v3.6 身份、SDK 分发、Raft schema、macOS ARM64 和 debug symbols 的 ADR、manifest、依赖顺序与默认阻断状态必须一致；ADR 接受只授权后续实现，不代表能力或发布资产已交付 | `scripts/check_next_minor_decisions.py`, `docs/decisions/v3.6-decision-manifest.json`, `docs/mainline-execution-plan.md`, `docs/current-state.md` |
...
| `script_inventory_governance` | stable | 88 个顶层脚本必须有维护分类和公开入口索引；新增脚本必须进入 `docs/script-inventory.json`，避免公开入口、producer、tool、legacy wrapper 继续混杂 | `scripts/check_script_inventory.py`, `docs/script-inventory.json`, `scripts/README.md`, `docs/current-state.md` |
...
| `validation_summary_contract` | stable | 核心生产证据 summary 必须保持 `summary_version=2`、`generated_at`、`overall_pass/passed`、`artifacts`；R0/long-soak/R4/R5/R6/JWKS rotation 还必须具备完整 provenance。正反向契约门禁覆盖跨 SHA、缺字段、缺时间、checkout 不匹配和 R3 双 summary 判定 | `scripts/check_validation_summary_contract.py`, `scripts/check_evidence_provenance_contract.py`, `scripts/render_validation_summary.py`, `docs/fixed-runner-playbook.md` |
...
- 下一 minor 决策治理：运行 `scripts/check_next_minor_decisions.py`，校验五项 ADR、机器可读 manifest、实现依赖和默认阻断口径；门禁通过不等价于对应实现、平台验证或发布资产通过。
