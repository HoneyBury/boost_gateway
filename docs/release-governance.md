...
| `production_auth_audit_gate` | stable | 生产模式不得默认使用 dev token fallback；legacy demo admin surface 的默认 ACL、拒绝审计和审计最小键必须由 release gate 检查，但这不代表当前 v2 主线提供正式 admin 控制面 | `scripts/check_security_release_gate.py`, `include/v2/login/login_backend_service.h`, `src/v2/login/login_backend_service.cpp`, `examples/v2_login_backend/main.cpp`, `include/game/gateway/admin_service.h`, `src/game/gateway/admin_service.cpp`, `docs/archive/history-v1/v1-admin-audit-rules.md`, `tests/unit/admin_service_test.cpp` |
...
- Per-commit perfromance smoke：`.github/workflows/perf-commit-check.yml`，仅 `workflow_dispatch` 手动触发。`perf` label 不再自动触发（参见 commit `3966850` 限制）。
