...
2026-05-12         2026-05-12         2026-05-12         2026-Q3
 ...
 发布基础设施     优雅降级         SLO/SLI 定义    协议规范
   ...
   2026-Q3          2026-Q4          2027+
       │                │                │
   安全加固         企业可靠性      分布式运行时
   可观测性         高级游戏特性    K8s Operator
   K8s 部署         性能优化        远程 Actor
...
| 编号 | 条目 | 说明 |
|---|---|---|
| H1 | 后端配置热加载 | `login_backend.json` / `room_backend.json` / `battle_backend.json` 支持 `ConfigWatcher::reload` |
| H2 | 断路器（Circuit Breaker） | `BackendConnection` 连续失败 ≥ 3 次 → 熔断 30s → 半开探测 → 恢复 |
| H3 | 服务优雅降级 | 某一后端不可用时 gateway 返回明确错误码，不影响其他后端路由 |
| H4 | 背压保护 | `Session` 写队列超 `max_pending_write_bytes` 时拒绝新消息，回压到上游 |
| H5 | 连接限制增强 | 按服务类型（login/room/battle）分别限制最大连接数 |
| H6 | 内存保护 | `ObjectPool` / `BumpArena` 增加可配置上限，超限告警不崩溃 |

**验收标准**：
- 全部 6 项有单元/集成测试覆盖
- 任意后端宕机不影响 gateway 对其他后端的正常路由
