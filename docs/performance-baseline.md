...
纠偏后的首个 focused run `29742852766`（`29fc4cff`）在 AOI runner 上固定 service CPU `0`、loadgen CPU `4-7`、loadgen I/O threads 4 和 Gateway `io_cores=4`。echo-5K 三轮 P99 均为 2ms，echo-10K 为 5/10/20ms；吞吐中位数分别为 58010.88/59251.73 msg/s，全部 0 rejected/failed。六轮 service 总 CPU 为 55.9%-62.5%，loadgen 为 153.7%-159.4%，均符合各自 affinity 物理上限。该结果说明旧单核 75/100ms 主要受到 loadgen 共核干扰，不支持直接调整 Gateway 线程默认值；完整 1/2/4 矩阵仍需在相同 loadgen 集合和 workload identity 下刷新。
