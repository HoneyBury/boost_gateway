...
| 维度 | 状态 |
|---|---|
| Linux capacity / business-capacity | v3.5.3 已闭环；发布后隔离矩阵候选 `375910f3` 三档均通过 |
| 2h soak | v3.5.3 冻结 SHA 已通过 |
| 8h soak | v3.5.3 冻结 SHA run `29711044558` 已连续通过 28801.652 秒 |
| 1/2/4 核资源线性 | 隔离后的完整三档矩阵已形成；当前 workload 未触及 CPU 扩展上限，不得使用线性推算值 |
...
纠偏后的首个 focused run `29742852766`（`29fc4cff`）在 AOI runner 上固定 service CPU `0`、loadgen CPU `4-7`、loadgen I/O threads 4 和 Gateway `io_cores=4`。echo-5K 三轮 P99 均为 2ms，echo-10K 为 5/10/20ms；吞吐中位数分别为 58010.88/59251.73 msg/s，全部 0 rejected/failed。六轮 service 总 CPU 为 55.9%-62.5%，loadgen 为 153.7%-159.4%，均符合各自 affinity 物理上限。该结果说明旧单核 75/100ms 主要受到 loadgen 共核干扰，不支持直接调整 Gateway 线程默认值，并为随后同 workload identity 的完整矩阵确定了隔离口径。

完整矩阵随后在候选 `375910f3ecc96bf9c6c4981818a3262a8387d15c` 上完成：1/2/4 CPU 来源分别为 runs `29790072882`、`29791850363`、`29793036782`。三档都固定 loadgen CPU `4-7`、4 个 loadgen I/O threads、Gateway `io_cores=4`、三轮完整 capacity/business-capacity case、16x10 业务操作和 3 个 SDK full-flow 客户端。聚合器的 72 项检查全部通过，`evidence_complete=true`、`all_workload_gates_passed=true`。
...
所有 case 均为 0 rejected/failed。2/4 CPU 没有形成吞吐提升，且 4 CPU 的逐轮服务 CPU 中位总量最高也只有约 126%（400% 物理上限），因此当前矩阵测到的是固定 offered load/场景速率的平台期，而不是 CPU 饱和后的扩展曲线。它可以证明 1 CPU 已满足当前门槛，并证明隔离证据完整；不能据此宣称线性扩展或调整默认 `io_cores`。下一轮性能实验应先提高且固定 offered load，确认服务侧接近分配 CPU 上限，同时继续保持 loadgen 隔离。
