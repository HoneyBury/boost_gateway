...
| 维度 | 状态 |
|---|---|
| Linux capacity / business-capacity | v3.5.3 历史发布产物保留；发布后旧隔离矩阵缺真实客户端 lifecycle，当前容量结论待 AOI 刷新 |
| 2h soak | v3.5.3 冻结 SHA 已通过 |
| 8h soak | v3.5.3 冻结 SHA run `29711044558` 已连续通过 28801.652 秒 |
| 1/2/4 核资源线性 | 新聚合与饱和选点入口已实现；旧三档矩阵不满足 lifecycle 契约，固定 runner 数据待刷新 |
...
旧产物中的 0 rejected/failed 和吞吐/P99 只描述实际发出的请求，不能补足未启动、未连接或未认证客户端。它既不能证明 1 CPU 满足原目标，也不能据此宣称线性扩展或调整默认 `io_cores`。下一轮先运行完整 closed-loop saturation curve，在 Gateway 达到显式 CPU 配额阈值且 loadgen 保有余量时选定固定 case，再分别执行 service CPU 和单核 `io_cores` 的 1/2/4 三轮单变量实验。
