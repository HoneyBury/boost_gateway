...
| `perf-regression.yml` | `release` | GCC 12 | 是 | `sccache-Linux-perf-regression-<config-hash>-<git-sha>` | smoke / baseline / capacity 统一性能入口；critical gate 失败会阻断 workflow |
...
- `release.yml`、`nightly-stability.yml`、`perf-regression.yml` 现已同步到与 `ci.yml` 一致的“配置哈希前缀 restore + commit exact key save”模式，避免旧 exact-hit 长期阻断新缓存保存。
- hashFiles 输入因 workflow 而异：`ci.yml` / `release.yml` 已纳入 Conan lock/profile/remotes；`nightly-stability.yml`、`perf-regression.yml` 继续采用与各自构建入口匹配的 CMake-only 分段模式，但 save 语义已统一为 per-commit exact key。
- 所有 workflow 的 `restore-keys` 仍以 `sccache-${{ runner.os }}-` 结尾，允许精确键未命中时回退到同一 OS 的任意历史缓存；当前 `ci.yml`、`perf-regression.yml`、`nightly-stability.yml`、`release.yml` 都已有 hosted 实跑证据。
...
> - `perf-commit-check.yml` 已退役；`perf-regression.yml` 以 `perf_preset=smoke` 覆盖原 smoke 用例。
...
> CI 基线入口：
> - 手动运行：`.github/workflows/perf-regression.yml`
> - 手动触发 release baseline：`.github/workflows/release.yml`
> - 手动 bounded stability：`.github/workflows/nightly-stability.yml`
