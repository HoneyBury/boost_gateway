...
| `linux-arm64` | ELF ARM64 | `linux/arm64` Docker Compose 或原生进程 | 生产候选；Conan、R5/R6、JWKS、SDK 与 ELF/debug-symbol exact-SHA evidence 已通过，等待 Release/R0、完整基线和发布资产 |
| `macos-arm64` | Mach-O ARM64 | 原生进程编排，重启 gateway 后复跑 SDK full-flow | 生产候选；同 SHA 构建、安装、原生 R5、有界 smoke、JWKS、osx-arm64 SDK 和 dSYM 已在线通过，等待完整基线、长稳/容量阈值和 notarized 发布资产 |
...
两种 ARM64 runner 的 strict-offline Conan、原生 R5、JWKS、SDK package 和符号资产
预冻结证据已经完成。当前迁移顺序变为：补齐 Linux ARM64 Release/R0 与两平台完整
性能/容量/长稳基线，再冻结一个 SHA，最后扩展 release manifest、notarization、线上
资产复验和按平台汇聚的 readiness decision。

分支 `codex/v36-arm-platform-evidence` 进一步把 `release.yml`、
...
archive 和三份 SPDX。该实现状态只表示 workflow 已可路由，不能替代尚未运行的
Release/R0/baseline/soak/capacity 或已发布资产证据。
...
这些结果不替代最终冻结 SHA 的三次 baseline 与长稳/容量证据。
