...
from scripts.tools.verify_release_cmake_consumer import docker_command, write_consumer_project
...
def test_docker_command_enforces_offline_read_only_inputs(tmp_path: Path) -> None:
    ...
    for path in (package, source, build):
        path.mkdir()
    command = docker_command("consumer:test", package, source, build, ["cmake", "--version"])
...
def test_toolchain_probe_is_strict_and_uses_versioned_compiler() -> None:
    script = Path("scripts/tools/verify_release_cmake_consumer.py").read_text(encoding="utf-8")
