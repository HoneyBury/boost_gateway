...
TEST(RuntimeMetricsExporterTest, RendersPrometheusAndJsonOutputs) {
    v2::diagnostics::RuntimeMetricsSnapshot snapshot;
    ...
    snapshot.prometheus_extension_text = "gateway_shadow_bridge_tracked_sessions 2\n";
    snapshot.json_extension_text = R"({"shadow_bridge":{"tracked_sessions":2,"active_sessions":1}})";
    ...
    const auto prometheus = v2::diagnostics::render_prometheus_metrics(snapshot);
    ...
    const auto json_text = v2::diagnostics::render_json_metrics(snapshot);
    ...
    const auto diagnostics = v2::diagnostics::render_diagnostics_metrics(snapshot);
    ...
    const auto diagnostics_json = v2::diagnostics::render_diagnostics_json_metrics(snapshot);
...
TEST(RuntimeMetricsExporterTest, WritesMetricsFilesToDisk) {
    v2::diagnostics::RuntimeMetricsSnapshot snapshot;
    snapshot.counters.accepted_sessions = 1;
    snapshot.active_sessions = 1;

    const auto temp_dir = std::filesystem::temp_directory_path() / "runtime_metrics_exporter_test";
    const auto prom_path = temp_dir / "gateway.prom";
    const auto json_path = temp_dir / "gateway.json";

    const auto ok = v2::diagnostics::write_metrics_files(
        snapshot,
        {
