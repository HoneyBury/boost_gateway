...
> - 当前发布版为 **`v2.0.0`**（2026-05-12），七大模块（M1-M7）全部落地，473 测试通过。
> - v2.0.0 整体评级为"高级"阶段（核心能力完整），下一阶段目标为 v2.3.0"企业"阶段（详见 `docs/v2-enterprise-roadmap.md`）。
> - **v1.x 能力成熟度以 `docs/v1-maturity-matrix.md` 为准**——该矩阵是维护期单一事实源。v1 文档仍作为兼容链路、桥接边界和历史回归的事实源。
