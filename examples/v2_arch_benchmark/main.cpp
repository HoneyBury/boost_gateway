...
LatencyStats run_instance_runtime_tick_all_latency(
    ...
    if (!runtime.set_event_callback([&snapshot_events](const v2::realtime::InstanceEvent& event) {
        if (event.type == v2::realtime::InstanceEvent::Type::kSnapshotAvailable) {
            ++snapshot_events;
        }
    })) {
        throw std::runtime_error("failed to configure realtime benchmark callback");
    }
